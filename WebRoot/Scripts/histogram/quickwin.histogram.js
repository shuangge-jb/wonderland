...
histogram.prototype.draw = function () {
    ...
    //动态设置 canvas 的尺寸
   this.ctx.canvas.height = Math.ceil(maxamount / this.yInterval) * 50 + 125;    //120 is for the chart title.
    ...
    for (var i = 0; i < dataarray.length; i++) {    
        this.ctx.beginPath();
        this.ctx.strokeStyle = colors[i];
        this.ctx.moveTo(this.ctx.canvas.width-92, 50+i*17);
        this.ctx.lineTo(this.ctx.canvas.width - 75, 50 + i * 17);
        this.ctx.font = "12px Arial";
        this.ctx.textAlign = "left";
        this.ctx.fillText(dataarray[i].category, this.ctx.canvas.width -70, 50 + i * 17+4);
        this.ctx.stroke();
    }
